...
if (!isset($ruta[1]) || $ruta[1]=="") {
    header("location:" . BASE_URL . "ambientes");
}
...
$this->Cell(0, 5, 'DIRECCIÓN DE ADMINISTRACIÓN', 0, 1, 'C');
        ...
        $this->Cell(0, 8, 'REPORTE AMBIENTES', 0, 1, 'C');
    }
...
if ($err) {
    ...
    <table class="cuerpo">
      ...
      <tbody>';
        ...
        foreach ($ambientes as $ambiente) {
            ...
            $contenido_pdf .= '<td>' . $ambiente->id . '</td>';
            $contenido_pdf .= '<td>' . $ambiente->nombreInstitucion . '</td>';
    ...
    $pdf->SetTitle('ambientes');
    $pdf->SetMargins(PDF_MARGIN_LEFT, 55, PDF_MARGIN_RIGHT);
    $pdf->SetAutoPageBreak(true, 10);
    $pdf->SetFont('helvetica', '', 12);
    $pdf->AddPage();
    $pdf->writeHTML($contenido_pdf, true, false, true, false, '');
    $pdf->Output('reporte-ambientes.pdf', 'I');
    exit;
}
